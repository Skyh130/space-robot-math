 ...
 *
 * 맞춰 둔 눈금:
 * - 전체 한 바퀴(월드 1~8, 스테이지 48개, 문제 388개)를 다 맞히면 1940코인.
 * - 상점을 통째로 사려면 3600코인. 한 바퀴로는 절반쯤 산다.
 * - 그래서 첫 완주로 예닐곱 개를 사고, 나머지는 별을 올리러 다시 하면서 채운다.
 *   한 번에 다 사면 다시 할 이유가 없고, 하나도 못 사면 모을 이유가 없다.
 * - 월드 4~8 을 열면서 벌이가 740에서 1940으로 늘었다. 값을 그대로 두면 첫
 *   완주에 상점이 통째로 비어 버려서, 물건 값과 품목 수를 함께 올렸다.
